...
void MeshRenderer::fillBuffers(std::vector < objl::Vertex >& vert, std::vector<unsigned int>& indi) {
    ...
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_TRUE, 8 * sizeof(float), (void*)(6 * sizeof(float))); //textcoords
...
void MeshRenderer::fillBuffers(std::vector<dc::Vertex>& vert, std::vector<unsigned int>& indi)
{
    indicesAmt = indi.size();
    glGenVertexArrays(1, &VAO);
    glBindVertexArray(VAO);
...
void MeshRenderer::drawInstanced(unsigned int count) const {
    ...
    glBindVertexArray(VAO);
    //draw elements stored in VAO
    glDrawElementsInstanced(GL_TRIANGLES, indicesAmt, GL_UNSIGNED_INT, 0, count);
}